...
class Site {
  ...
  factory Site.fromJson(Map<String, dynamic> json) => Site(
        ...
        createdAt: DateTime.tryParse(json['createdAt'] as String? ?? '') ??
            DateTime.now(),
        drawingType:
            DrawingType.values.byName(json['drawingType'] as String? ?? 'blank'),
...
class Defect {
  ...
  factory Defect.fromJson(Map<String, dynamic> json) => Defect(
        ...
        category: DefectCategory.values
            .byName(json['category'] as String? ?? 'generalCrack'),
        normalizedX: (json['normalizedX'] as num? ?? 0).toDouble(),
        normalizedY: (json['normalizedY'] as num? ?? 0).toDouble(),
        details:
            DefectDetails.fromJson(json['details'] as Map<String, dynamic>? ?? {}),
      );
}
...
class _HomeScreenState extends State<HomeScreen> {
        ...
        return SafeArea(
          child: Column(
            ...
            children: [
              ...
              ListTile(
                leading: const Icon(Icons.grid_blank_outlined),
                title: const Text('Create blank canvas'),
                subtitle: const Text('Start with a clean sheet'),
                onTap: () {
                  Navigator.of(context).pop(
                    _DrawingSelection(type: DrawingType.blank),
                  );
                },
              ),
            ],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Inspection Sites'),
      ),
      body: _isLoading
...
class _DrawingScreenState extends State<DrawingScreen> {
  ...
  Future<void> _handleTap(TapDownDetails details) async {
    ...
    final normalizedX =
        (scenePoint.dx / _canvasSize.width).clamp(0.0, 1.0);
    final normalizedY =
        (scenePoint.dy / _canvasSize.height).clamp(0.0, 1.0);
    ...
    final countOnPage =
        _site.defects.where((defect) => defect.pageIndex == _currentPage).length;
